...
Class Add_employee
{
function add_employees($common,$db_object,$user_id,$default,$emp_id=null,$pos_id=null)
{
$path=$common->path;
...
$selqry="select $fieldofuser from $user_table where user_id='$emp_id' and admin_id='$user_id'";
...
if($user_id!=1)
{
	$selqry="select user_id,username from $user_table where admin_id='$user_id'  order by user_id";
...
{
	$temp_id=$usernames[$i]["user_id"];
	$usernames[$i]["username"]=$common->name_display($db_object,$temp_id);
}
...
echo $employee_id;
$empobj->add_employees($common,$db_object,$user_id,$default,$emp_id,$pos_id);
include("footer.php");
?>
