...
//-searches for the activities that are corresponding to that object id if	it is not there empty boxess are printed
	...
	//echo $objset[0]["objective_name"];
	$obv=$objset[0]["objective_name"];
	echo "<br>";
	...
	$filename="../templates/career/activities_panel.html";
...
/*
	...
	{
$bld_id=$build_id;
echo "<script language='javascript'>window.location.replace('skills_name.php?build_id=$bld_id')</script>";
exit;
	}
...
//----if there isno activities for that object id then in inserts the acitivites else replaces that		
