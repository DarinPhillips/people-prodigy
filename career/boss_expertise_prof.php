...
class profile
{
	function expertise_profile($db_object,$common,$user_id,$default,$post_var)
	{
		...
		$path=$common->path;
		$xtemplate=$path."templates/career/boss_expertise_prof.html";
		...
		$qry="select round(avg(label_id)) as rater_label_no,
				rated_user,skill_id 
				from $other_raters_tech 
				where rated_user in $users_id 
		...
		$selqry2="select rater_labelno from $rater_label_relate,$skill_raters where
				...
				order by rater_labelno desc limit 1";
		...
		for($a=0;$a<count($percent1);$a++)
		{
			...
			$skill_name=$skill_name1[$key];
			
			$percent=$percent1[$key];
...
$obj->expertise_profile($db_object,$common,$user_id,$default,$post_var);
