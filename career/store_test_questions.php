...
DESCRIPTION:
This script stores all the Test information into the database
which can be later edited.

---------------------------------------------*/
include("../session.php");

class storeTest_questions
{
	...
	for($i=0;$i<count($q_arr);$i++)
		...
		list($un,$qid)=split("_",$key);
			...
			$mysql = "insert into $quest_table set test_id = '$test_id',question = '$val',score='$m_array[$i]',skill_id='$s_array[$i]',user_id='$user_id'";
...
//update the assign test builder table so that the skill name is not displayed in the alert board once the admin approves it...
	...
	$mysql = "update $assign_test_builder set status = 'a' where user_id = '$user_id' and group_id = '$group_id'";
	//echo $mysql;
	$db_object->insert($mysql);
...
if($fSave_later)
	...
	$obj->saveQuestions($db_object,$common,$post_var,$user_id,$error_msg,$default);
	header("Location:show_all_questions.php?test_id=$test_id&group_id=$group_id");
	}
else
	{
	header("Location:front_panel.php");
