...
else
{
	...
	for($i=0;$i<count($q_arr);$i++)
		...
		list($un,$qid)=split("_",$key);
			...
			$mysql = "insert into $quest_table set test_id = '$test_id',question = '$val',score='$m_array[$i]',skill_id='$s_array[$i]'";
...
if($fSave_later)
	...
	if($test_mode == "approve")
	{
		header("Location:approve.php?test_id=$test_id&group_id=$group_id");
	}
	else
	{
	header("Location:show_unapproved.php?test_id=$test_id&test_mode=$test_mode&group_id=$group_id");
	}
	}
else
	{
