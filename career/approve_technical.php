<?php
/*---------------------------------------------
SCRIPT:approve_technical.php
AUTHOR:info@chrisranjana.com	
UPDATED:17th Sept

DESCRIPTION:
This script approves the test.
...
class showAll_questions
{
	...
	while(list($kk,$vv)=@each($post_var))
		...
		$xPath=$common->path;
		$xTemplate=$xPath."/templates/career/approve_technical.html";
		$returncontent=$common->return_file_content($db_object,$xTemplate);
		

if($user_id==1)
{
	if($test_mode != "approved")
	{
...
//check if the test already exists in the approved list of tests...
...
for($i=0;$i<count($ref_previous_arr);$i++)
{
	...
	$unapp_test_name1 = mysql_escape_string($unapp_test_name);

	if($ref_previous_id == $unapp_test_id)
	{
		
	$mysql = "select test_id from $tests where test_name='$unapp_test_name1' and test_type='$unapp_test_type' and tot_question='$unapp_tot_question' and user_id='$unapp_user_id'";
	//echo $mysql;exit;
...
//check over.	
	...
	$mysql = "insert into $tests set test_id='',test_name='$unapp_test_name1',test_type='$unapp_test_type',tot_question='$unapp_tot_question',user_id='$unapp_user_id',date=now(),ref_test_id='$unapp_test_id'";
	...
	$mysql = "select $fields from $unapp_skill_percent_table where test_id='$app_testid' order by level";
	...
	for($i=0;$i<count($skillpercent_arr);$i++)
		...
		$mysql = "insert into $skill_percent set test_id='$test_id',skill_id='$skill_id',level='$level',percent='$percent'";
		$db_object->insert($mysql);
...
function delete_test($db_object,$common,$post_var,$user_id,$error_msg,$default)
{
	...
	$mysql = "delete from $assign_test_builder where skill_id = '$skill_used_id' and user_id = '$user_id'";
	//echo "$mysql<br>";
	$db_object->insert($mysql);
