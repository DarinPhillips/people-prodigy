...
class userplan
	{
		...
		{
			$path = $common->path;
			$filename = $path."templates/performance/approve_objective.html";
			...
			$rejected = $common->prefix_table("unapproved_rejected_category");
			if($uid!="")
			{
				$user_id=$uid;
			}			
...
//			print_r($gettopboss);
			...
			//check whether teh user is admin or not
		...
		function rejected_plan($db_object,$common,$default,$user_id,$uid,$_POST)
		{
			...
			$reject = $common->prefix_table("unapproved_rejected_category");
			$del_check = 0;
			$key_array = array();
			$reject_array = array();
			while(list($key,$value)=each($_POST))
			...
			header("location: approve_objective2.php?emp_id=$user_id");
		}//end reject	
	}//end class
	$ob = new userplan;
	if($next!="")
