...
{
	function alert_approved_appraisal($db_object,$common,$user_id,$error_msg)
	{
		...
		$xtemplate=$path."templates/performance/approved_appraisal_admin.html";
		...
		if($result[0]=="")
		{
			echo $error_msg['cPresentlyNoAppraisal'];
	...
	function show_approved_appraisal($db_object,$common,$user_id,$approved_on,$default)
	{
		...
		$fUser=$result[0][user_id];
		
		$boss_id=$result[0][boss_id];
		
		$path=$common->path;
		
		$xtemplate=$path."templates/performance/view_approved_appraisal_admin.html";
		...
		$cnt=$ret_array[9];
			...
			for($j=0;$j<$cnt;$j++)
			{
				...
				if(count($exp_res)==0)
				{
					$exp="<tr><td class=code><TEXTAREA WRAP=PHYSICAL ROWS=3 COLS=50 name=$fRater_exp readonly>$explain</TEXTAREA></td></tr>";
				}
