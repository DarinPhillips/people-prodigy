...
function download_file($db_object,$common,$approved_on,$default,$err,$user_id,$_POST)
	{
		...
		$xTemplate=$path."performance/approved_appraisal/appraisal.txt";
		...
		$val[user_summary]=$_POST[fUser_final];
		
		$val[user_comment]=$_POST[fUser_comment];
		...
		$file = $path."performance/approved_appraisal/appraisal_$user_id.txt";
		...
		if(file_exists($file))
		{
			...
			$len  = filesize($file);
			$filename = "appraisal_$user_id.txt";
