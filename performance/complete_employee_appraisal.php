...
class appraisal
{
	function show_alert($db_object,$common,$user_id)
	{
		...
		$sql="select $assign_performance_appraisal.status,$user_table.username,$assign_performance_appraisal.dummy_id from $user_table,
		
		$assign_performance_appraisal where $assign_performance_appraisal.user_id=$user_table.user_id
		
		and $assign_performance_appraisal.boss_user_id='$user_id' and $assign_performance_appraisal.check_status<>'c'";
		...
		$xTemplate=$xPath."templates/performance/alert_appraisal.html";
		
		$content=$common->return_file_content($db_object,$xTemplate);
		
		preg_match("/<{alert_loopstart}>(.*?)<{alert_loopend}>/s",$content,$match);
		...
		$content=preg_replace("/<{alert_loopstart}>(.*?)<{alert_loopend}>/s",$str,$content);
	...
	//==========================================================================================
	function view_form($db_object,$common,$default,$user_id,$dummy_id,$gbl_met_value,$error_msg)
	{
	...
	//table declaration
		...
		$filename = $path."templates/performance/employee_appraisal.html";
...
//	print_r($perqry1);exit;
		...
		$match=$match[0];
			...
			for($j=0;$j<count($selres);$j++)
			{
				...
				$boss_qry="select rater_comment,rater_id from $performance_appraisal where user_id='$user_id'
				
				and o_id='$o_id' and who='b'";
				...
				//echo $boss_comment;exit;
				$dumm=$dummy_id-1;
				
				$qry="select rater_comment from $performance_appraisal where appraisal_id='$dumm' and who='r' and o_id='$o_id'";
				//echo $qry;exit;
				...
				for($k=0;$k<count($perres1[$j]);$k++)
				{
					...
					if($res_qry[0]!="")
					{
						$rater_comment=$res_qry[$k];
					}
					else
					{
					$qry="select effective from $approved_feedback where raters_id='$rater_id' and o_id='$o_id'";
					
					$result=$db_object->get_a_line($qry);
					
					$rater_comment=$result[effective];
					}
				...
				$exp_qry="select rater_summary,rater_point from $appraisal_results where
				
				  user_id='$user_id' and o_id='$o_id'";
			...
			$total_points=$metexpectation*$j;							
		$file=preg_replace("/<{objective_loopstart}>(.*?)<{objective_loopend}>/s",$obj_replace_full,$file);
		
		$final_qry="select final_summary,final_point2 from $appraisal_results where
				
		user_id='$user_id' and appraisal_id not in ('$dummy_id') group by appraisal_id";
				//echo $rater_qry;exit;
		...
		$xArray[boss_id]=$boss_id;
				
		$xArray[user_id]=$user_id;	
		
		$xArray[appraisal_id]=$dummy_id;
		
		$xArray[final]=$final;
		
		$xArray[total_points]=$total_points;
		
		$xArray[total_points1]=$final_point2;
		
		$xArray[username]=$common->name_display($db_object,$user_id);
...
//----------------------------------------------------------------------------------------	
	function view_form_from_table($db_object,$common,$user_id,$dummy_id,$default,$gbl_met_value,$error_msg)
	{
		...
		$cnt=$ret_array[9];
			...
			for($j=0;$j<$cnt;$j++)
			{
				...
				$boss_qry="select rater_id,rater_comment from $performance_appraisal where user_id='$user_id'
				...
				$exp_qry="select rater_summary,rater_point from $appraisal_results where
				  ...
				  user_id='$user_id' and appraisal_id not in ('$dummy_id')";
				//echo $rater_qry;exit;
				$exp_res=$db_object->get_rsltset($exp_qry);
				$exp="";
				$c=count($exp_res)-1;
...
switch($action)
{
	...
	$qry="update $assign_performance_appraisal set check_status='c' where dummy_id='$dummy_id'";
	
	$db_object->insert($qry);
	
	$sql="select * from $performance_appraisal where appraisal_id='$dummy_id'";
	...
	if($res[0]=="")
	{
		...
		$obj->view_form($db_object,$common,$default,$user_id,$dummy_id,$gbl_met_value,$error_msg);
	}
	else
	{
		$obj->view_form_from_table($db_object,$common,$user_id,$dummy_id,$default,$gbl_met_value,$error_msg);
	}
