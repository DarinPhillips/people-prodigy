...
class userplan2
	{
	function view_plan($db_object,$common,$default,$user_id,$uid,$gblfreq,$gblpercent,$s_id,$post_var)
		{
			if($post_var=="")
			{
				$post_var=array();
			...
			$filename = $path."/templates/performance/userplan2_input.html";
			...
			if(ereg("^fObjective_new_",$key))
			{
				...
				{//check null loop
					...
					{//check id loop					
						...
						committed_no='$committed',percent ='$percent',
						how_to_get_$default='$show',submit_date=now(),changed_user_id='$changed_user'";
	...
	if($back!="")
	{
		header("location: userplan1_input.php?uid=$emp_id");
	}
	if($save!="")
	{
	...
	{
		$ob->save_objective($db_object,$common,$default,$user_id,$emp_id,$post_var);
		header("location: userplan3_input.php?uid=$emp_id");
	}
