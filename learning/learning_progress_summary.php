...
{	
	...
	function display($db_object,$common,$user_id,$default,$error_msg,$learning,$dr_id)
	{
		...
		if($dr_id!="")
		{
			...
			$dir_rep = $common->get_chain_below($position,$db_object,$twodarr);
			$dir_ids = $common->get_user_id($db_object,$dir_rep);
			for($i=0;$i<count($dir_ids);$i++)
			{
				$dir_array[] = $dir_ids[$i][user_id];
			}
			
			if(in_array($dr_id,$dir_array))
			{
				$user_id = $dr_id;
			}
			else
			{
				echo $error_msg['cSorrynotallowed'];
				include ("footer.php");
				exit;
			}
		}
		...
		$path		=$common->path;
		$template	=$path."/templates/learning/learning_progress_summary.html";
		...
		if($dr_id=='')
		{
			$returncontent = preg_replace("/<{others_loopstart}>(.*?)<{others_loopend}>/s","",$returncontent);
		}
		else
		{
			$mail = "select email from $user_table where user_id='$dr_id'";
	...
	$obj->display($db_object,$common,$user_id,$default,$error_msg,$learning,$dr_id);
