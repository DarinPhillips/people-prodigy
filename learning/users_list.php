...
class raters
{
	...
	$mysql="select  distinct(user_id),$approved_devbuilder.skill_id ,cdate 
		from $approved_devbuilder $joinclause $dateclause
			 where pstatus='a' 
		and url in ('$split')  and interbasic_id='14' group by $approved_devbuilder.skill_id ,user_id order by cdate";
