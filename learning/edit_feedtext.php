 ...
 function display_mail_option($common,$db_object,$user_id)
  {
	...
	$path=$common->path;
	$xFile=$path."/templates/learning/edit_feedtext.html";
	$xTemplate=$common->return_file_content($db_object,$xFile);
	$config=$common->prefix_table("config");
	$query="select feedback from $config";
	$result=$db_object->get_a_line($query);

	$values["directreplace"]["dev_text"]=$result["feedback"];
...
function updatemail($common,$db_object,$form_array)
   {
   	...
   	$insqry="update $config set feedback='$fdev_text' where id=1";
