...
class update_plan
{
	...
	function display($db_object,$common,$user_id,$default,$error_msg,$learning,$uid)
	{
		...
		$path		=$common->path;
		$template		=$path."/templates/learning/updated_plan_progress_approval.html";
		...
		$mysql 	= "select $skill_table.skill_name,$skill_table.skill_type,$plan_table.skill_id,$plan_table.title,
					...
					$plan_table.user_id='$uid' and $plan_table.update_status='u'
				...
				for($j=0;$j<count($dBasic);$j++)
				{
					...
					for($k=0;$k<count($dInterbasic);$k++)
					{
						...
						$mysql="select * from $plan_table where interbasic_id='$ib_id' 
							and skill_id='$skills' and user_id='$uid'and title!=''
							 and pstatus='a'";
	...
	function update($db_object,$common,$post_var,$user_id,$error_msg,$default,$learning)
	{
		...
		/* FOR UPDATING COMPLETED BY DATE */
			for($i=0;$i<count($keys);$i++)
			{
				...
				$mysql = "update  $plan_table set completed_date='$completed_date',update_status='a'
						   where build_id='$build_id' and user_id='$uid' and pstatus='a' and update_status='u'";				
...
if($return)
{
	...
	include("header.php");
	$obj->display($db_object,$common,$user_id,$default,$error_msg,$learning,$uid);
